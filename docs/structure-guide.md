...
### 訊息系統（17 個檔案）
**做什麼**：站內訊息通知、用戶私訊對話與管理員群發公告。
**包含功能**：
- 收件匣：檢視收到的通知與系統訊息
- 私訊對話：從用戶卡片開啟私訊，支援已讀、編輯、撤回與對話搜尋
- 管理員廣播：向全體或特定用戶發送站內信
- 聊天室稽核：super_admin 在日誌中心查看私訊使用 log 與指定用戶對話稽核副本
...
| 輪播廣告 | 首頁上方自動輪播的圖片廣告 |
| 系統公告 | 首頁公告欄，顯示最新消息 |
| 運動快速入口 | 依活動數量排序的運動分類入口，點擊後直接切到活動頁並套用運動篩選 |
| 首頁數量儀表 | 顯示活動數、俱樂部數、賽事數與已記錄瀏覽數；活動數排除已取消、私密、俱樂部限定與開始時間已過的活動 |
| 首頁排版順序 | 首頁管理可調整首頁容器順序，設定存於 `banners/home-layout` |
| 我要開團 | 首頁明顯但不佔版面的建立活動入口，登入後會帶到活動建立流程 |
| 彈窗廣告 | 進站時彈出的全屏廣告或公告 |
...
| 錯誤日誌 | 記錄系統錯誤，提供白話摘要、嚴重程度、同類聚合、近 7 天趨勢、診斷包複製與裝置/版本/context 等資訊 |
...
- 舊網址仍保留:`#page-xxx`、`?event=xxx`、`?team=xxx`、`?tournament=xxx`、`?profile=xxx` 都繼續有效。2026-05-21 最終規則為根網址 `?event=xxx`、`?team=xxx`、`?tournament=xxx`、`?profile=xxx` 仍由 `index.html` 中繼跳 Mini App；活動社群分享請用 `/event-share/{id}`，crawler 留在 OG 頁讀縮圖，真人自動進網站 `/events/{id}`。
