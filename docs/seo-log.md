...
**實作內容**：
1. 將 `/seo/football-taichung` 的 title、description、OG、Twitter、WebPage JSON-LD 改為明確承接「台中足球場推薦、朝馬足球場、西屯足球場、室內五人制」搜尋意圖。
2. 在 `/seo/football-taichung` 加入「台中市足球場先看哪裡」快速答案、朝馬 / 西屯選擇段落、延伸閱讀內鏈，以及 3 組可見 FAQ；同步更新 FAQPage JSON-LD。
3. 補強 `/blog/taichung-football-field-rental-guide`，加入朝馬足球場、西屯足球場、台體大與西屯室內五人制對照表，並把開團頁面資訊需求串回 ToosterX 報名場景。
4. 在 `/seo/football` 中部段落補入指向 `/seo/football-taichung` 與台中足球場租借 blog 的精準內鏈，集中傳遞場地詞群權重。

**決策備註**：
- 本輪不另開「朝馬足球場」單頁，先用既有台中足球落地頁承接，避免低樣本詞被拆散。若後續 GSC 顯示「朝馬足球場」曝光連續成長且平均排名仍卡在 8-15，再考慮新增場地型子頁。
- GSC 的第一頁判斷採平均排名，低曝光詞仍需觀察 2-4 週；後續重點看 impressions 是否擴大、CTR 是否提升、第二頁候選詞是否進入平均前 10。
...
**修正內容**：
1. 移除 `scripts/gsc-snapshot.js` 內不存在的 `https://toosterx.com/blog/pickleball-rules` inspection seed。
2. 擴充 `submit-sitemap.yml` path filter，新增 `blog/**`、`roles/**`、`privacy.html`、`terms.html`。
3. 將 `lighthouse.yml` 目標改成 clean URL，補入 `/seo/nantun-football-park`、`/blog/`、代表性 blog 文章與 `/roles/`。
4. 補齊 `privacy.html`、`terms.html` 的 `meta description`、`hreflang` 與 Twitter Card。
5. 補齊 `roles/index.html` 的 hreflang、OG image/url/locale/site_name、Twitter Card，以及 `BreadcrumbList` + `WebPage` JSON-LD。
6. 將 `roles/` 納入 SEO JSON-LD 單元測試掃描範圍，避免角色頁結構化資料回歸時漏測。
7. 補齊所有 `blog/**` 與 `seo/**` 公開頁的 Twitter Card title/description/image，統一沿用 OG metadata。
8. 新增 `tests/unit/seo-metadata.test.js`，檢查公開可索引頁的 title、description、canonical、hreflang、OG、Twitter Card 與 JSON-LD 覆蓋。

**次要項目審計**：
1. Blog 頁多數共用 `og.png`，SEO 著陸頁多數共用 app icon；若要提升社群 CTR，後續應規劃分類或頁面專屬 OG 圖。
2. 首頁、SEO 著陸頁與 Blog 文章仍大量使用 `meta keywords`；Google 已不以此作為主要排名訊號，後續可逐步降權或移除，避免維護成本與 keyword stuffing 觀感。
3. Cloud Functions 動態活動/隊伍分享頁仍輸出 `noindex,nofollow`；若要爭取自然搜尋流量，應另建可索引的活動聚合頁，而不是直接開放私人或短期動態分享頁索引。
4. 最新 GSC 成效需透過 `/admin/seo` 或 GitHub Actions secret 執行 `scripts/gsc-snapshot.js` 驗證；本輪只修正程式與 metadata，未連線抓取最新 Search Console 指標。
...
**修改內容**：
1. 補上「SEO 待辦 / 警示」、「SEO 頁面機會」、「品牌 / 非品牌查詢」、「Search Appearance」的說明按鈕與說明內容。
2. 將總覽、每日趨勢、頁面表現、裝置、國家、搜尋類型、前 20 名查詢詞、熱門搜尋詞、Sitemap、URL 收錄狀態的說明改成白話口語風格。
3. 同步更新舊有的空狀態、警示提示、表格欄位與判讀提醒，避免同一頁出現「新說明白話、舊提示偏工程術語」的不一致。
4. 說明內容改為強調「這代表什麼」、「需不需要緊張」、「下一步可以怎麼做」，降低 GSC 術語造成的誤解。

---

### 2026-04-29 — SEO 後台決策化優化

**目的 / 背景**：SEO 後台原本偏資料展示，且「前兩頁可見關鍵詞」容易讓低樣本 GSC 曝光被誤解成穩定排名。

**修改內容**：
1. SEO 後台新增「SEO 待辦 / 警示」，自動提示資料過舊、URL 收錄異常、Sitemap 警告、高曝光低 CTR、第二頁候選詞。
2. 將「前兩頁可見關鍵詞」改為「GSC 近 90 天平均前 20 名查詢詞」，並新增樣本可信度：樣本極少、樣本不足、較可信。
3. 新增「SEO 頁面機會」與「品牌 / 非品牌查詢」區塊，讓後台能區分既有品牌搜尋與自然 SEO 成長。
4. 新增 Search Appearance 顯示區塊，承接 GSC 已抓取但前台未展示的資料。
5. URL Inspection 快照改為合併固定清單與 `sitemap.xml`，並補入 `/blog/` 相關內容頁；檢查上限設為 45 個 URL，避免超量。
6. GSC query rowLimit 從 50 提升到 250，降低長尾查詢詞過早被截掉的機率。
7. 手機窄版表格增加橫向捲動與最小寬度，避免 SEO 表格欄位互相擠壓。

**判讀提醒**：GSC 平均排名不等於手動 Google 搜尋固定排名；實際搜尋會因地區、裝置、語言、時間與個人化而浮動。低曝光詞必須搭配樣本可信度判斷。
...
| roles/index.html | `BreadcrumbList`（2 層）+ `WebPage` |
...
### Meta 標籤配置
| 標籤 | 首頁 | SEO 著陸頁 | Blog | roles | privacy/terms |
|------|------|-----------|------|-------|---------------|
| title | 完整含關鍵字 | 完整含關鍵字 | 完整含關鍵字 | 有 | 簡單品牌標題 |
| meta description | 有 | 有 | 有 | 有 | 有 |
| meta keywords | 有 | 有（40+ 長尾詞） | 有（文章關鍵詞） | 無 | 無（不需要） |
| canonical | 有 | 有（clean URL） | 有（clean URL） | 有 | 有 |
| hreflang zh-TW | 有 | 有 | 有 | 有 | 有 |
| hreflang x-default | 有 | 有 | 有 | 有 | 有 |
| og:title/description/image/url/type/locale/site_name | 完整 | 完整 | 完整 | 完整 | 完整 |
| twitter:card/title/description/image | 完整 | 完整 | 完整 | 完整 | 完整 |
...
| `/seo/football` | 0.8 | 2026-04-22 |
| `/seo/basketball` | 0.8 | 2026-04-22 |
| `/seo/pickleball` | 0.8 | 2026-04-22 |
| `/seo/dodgeball` | 0.8 | 2026-04-22 |
| `/seo/running` | 0.8 | 2026-04-22 |
| `/seo/hiking` | 0.8 | 2026-04-22 |
| `/seo/football-taichung` | 0.8 | 2026-04-28（場地對照表新增） |
| `/seo/nantun-football-park` | 0.8 | 2026-04-22 |
| `/seo/sports-changhua` | 0.8 | 2026-04-22 |
| `/seo/sports-nantou` | 0.8 | 2026-04-22 |
...
index.html (noscript)
  ...
  │     └── /seo/hiking
  ├── 依地區揪團
  │     ├── /seo/football-taichung
  │     │     └── /seo/nantun-football-park
  │     ├── /seo/sports-changhua
  │     └── /seo/sports-nantou
  └── 運動百科 / Blog
        └── /blog/
              └── /blog/football-shoes-guide
...
| 觸發條件 | `sitemap.xml`、`seo/**`、`blog/**`、`roles/**`、`index.html`、`privacy.html`、`terms.html` 變更時觸發 | ✅ 已設定 |
...
6. **欄位說明彈窗新增（參考教學俱樂部 _showEduInfoPopup）**
   - 10 個 `seo-info-btn`（? 圖示）對應 10 個 section（2026-04-29 已擴充為 15 個說明入口，見上方「SEO 後台說明文案口語化」紀錄）
   - `_showSeoInfoPopup(type)` 函式含 10 種說明文案（2026-04-29 已擴充為 15 種，並改為口語判讀風格）
