...
> 最後修訂：2026-03-19（E8/E9 補完施作計劃、新增附錄 C 合併依賴的 8+3 步施作計劃）
...
| 30 | `js/modules/popup-ad.js` | `showPopupAdsOnLoad()` | ❌ 無防護（函式內部有 try/catch 包 localStorage，但外部呼叫無防護） |
...
### 5.1 Service Worker `addAll` 靜默失敗 → 快取雪崩（嚴重度：🔴🔴 P0 — 已確認存在 bug）
...
**建議修復方向（三管齊下）**：
1. **立即移除 `./js/core/mode.js`**（不存在的檔案）— 這是阻斷性 bug
2. 改為逐個 `cache.add()` + 個別錯誤處理，讓成功的資源至少被快取
3. 讓 STATIC_ASSETS 的 URL 與 index.html 一致（帶 `?v=`），或在 fetch handler 中做 cache key 正規化（去掉 `?v=` 後查詢）

### 5.2 onSnapshot 重連風暴 — 缺少 jitter（嚴重度：🟡 P1）
...
**建議修復方向**：
```javascript
// 加入 jitter：在 delay 的基礎上加上 0~30% 的隨機偏移
...
### 5.4 localStorage 配額耗盡靜默失敗（嚴重度：🟡 P1）
...
**建議修復方向**：
- 寫入失敗時 log warning（不要靜默吞掉），加入 `console.warn('[LS] quota exceeded for', name)`
- 加入 LRU 淘汰策略：寫入失敗時，主動刪除低優先集合（如 `newsArticles`、`gameConfigs`）的 localStorage，騰出空間給高優先集合
- 加入 quota 監控：`navigator.storage.estimate()` 或 try-write-check 模式
- 超限時降級為純記憶體模式（跳過 localStorage），避免反覆的無效寫入嘗試

### 5.5 `_persistCache()` 的 30 秒 debounce — 頁面關閉時丟失資料（嚴重度：🟡 P2）
...
**已驗證**：`firebase-service.js` 中 **沒有 `beforeunload` 或 `pagehide` 事件處理器**。`destroy()` 函式（line 2164-2202）需要手動呼叫，不會在頁面關閉時自動觸發。

**建議修復方向**：
```javascript
// 在 _setupVisibilityRefresh() 或 init() 中加入：
window.addEventListener('pagehide', () => {
  clearTimeout(this._persistDebounceTimer);
  this._persistCache();  // 頁面關閉前強制持久化
});
// 注意：用 pagehide 而非 beforeunload，因為 iOS Safari 不可靠觸發 beforeunload
```
...
**現狀**：
- `sw.js:9` → `CACHE_NAME = 'sporthub-20260319r'`（SW 獨立版本）
- `js/config.js` → `CACHE_VERSION = '20260320e'`（前端快取版本）
- 兩者獨立管理，格式不同（SW 用 `r` 後綴，config 用字母後綴）
...
### 5.12 App.init() 無錯誤邊界 → 單一失敗拖垮全局（嚴重度：🟡 P1 — 第六次驗證新增）
...
**建議修復方向**：
```javascript
// Phase A 之後額外加入：在 renderAll() 前加 try-catch 包裹非核心區段
try {
  this.initPwaInstall?.();
...
### 5.13 FirebaseService.init() 重複呼叫 → 監聽器洩漏（嚴重度：🟡 P1 — 第六次驗證新增）
...
### 5.17 ApiService-FirebaseService 競態 — renderAll 可能在 _cache 填充前執行（嚴重度：🔴 P1 — 第十次驗證新增）
...
**建議修復方向**：確認 `ensureCloudReady()` 完成後是否已有重新渲染機制（如 `_notifyDataReady` 回調），若無則需補上。

### 5.18 page-loader.js 缺少 HTTP 錯誤恢復（嚴重度：🟡 P1 — 第十次驗證新增）
...
**建議修復方向**：
```javascript
// page-loader.js — 增加錯誤處理
const response = await fetch(url);
if (!response.ok) {
  console.warn(`[PageLoader] ${name} load failed: HTTP ${response.status}`);
  // 可選：顯示「載入失敗，點擊重試」的用戶提示
  return;
}
```
...
|----------|------|---------|------|--------|
| E1a | 移除 sw.js 幽靈檔案 `mode.js` | **SW cache 完全失效的阻斷性 bug** | 零 | 🔴 立即 |
| E1b | SW addAll 改逐個快取 | 首次訪問 / SW 更新時的快取成功率 | 低 | 🔴 立即 |
| ~~E1c~~ | ~~SW fetch handler cache key 正規化~~ | ⛔ **第六次驗證撤回：會破壞 `?v=` 快取版本策略** | — | ~~🟡~~ 撤回 |
| E1c-alt | SW install handler 帶版本號（可選） | STATIC_ASSETS cache 命中率 | 低 | 🟢 可選 |
| E2 | onSnapshot 加 jitter | 50+ 用戶同時在線的連線穩定性 | 極低 | 🟡 |
| E3 | LS 寫入告警 + 保守淘汰 | 長期用戶的快取完整性 | 低 | 🟡 |
| E4 | pagehide 持久化 | LINE WebView 切頁後的資料新鮮度 | 極低 | 🔴 P1（第七次驗證升級） |
| ~~E5~~ | ~~Firebase SDK 載入失敗降級~~ | ⚠️ **第七次驗證撤回：5.11 前提錯誤，已有檢查** | — | ~~🔴~~ 撤回 |
| E6 | init() 非核心區段加 try-catch | 單一模組失敗拖垮全局 | 極低 | 🟡（Phase A 之後獨立 commit） |
| E7 | FirebaseService.init() 防重複呼叫 | 慢網路下的監聽器洩漏 | 極低 | 🟡 |
| E8 | ensureCloudReady 後確保重新渲染 | 首次訪問首頁空白閃爍（5.17） | 低 | 🔴 P1 |
| E9 | page-loader.js HTTP 錯誤檢查 | CDN 過載時頁面片段載入失敗（5.18） | 極低 | 🟡 P1 |
...
大量用戶同時湧入
    ...
    │
    ├─→ 所有用戶（不分首次/回訪）
    │   └─→ ⛔ sw.js STATIC_ASSETS 含不存在的 mode.js → addAll 每次必失敗（5.1 問題 B）
    │       └─→ SW cache 永遠為空 → cache-first 策略全部 miss → 靜態資源每次都從網路載入
    │           └─→ 即使不是高併發，每個用戶每次訪問都多出 36 次 HTTP 請求
    │
    ...
    │   │   └─→ onSnapshot 斷線 → 所有用戶同時觸發重連（無 jitter）（5.2）
    │   │       └─→ 重連風暴 → 進一步加重 Firestore 壓力 ──→ 惡性循環
    │   │
    │   ├─→ 回應延遲 > 6 秒 → init() 逾時
    │   │   └─→ _cache 不完整 → 前端渲染空白 → 用戶手動刷新 → 再次觸發全量查詢
    │   │
    │   └─→ init() 慢 → 用戶不耐 → 快速切頁或重整
    │       └─→ FirebaseService.init() 被重複呼叫（5.13）
    │           └─→ 雙重監聽器 → Firestore 讀取量加倍 + 記憶體洩漏
    │
    ├─→ LINE WebView 用戶
    │   ├─→ localStorage 配額 ~1MB（5.15）→ 只能快取 5-6 個集合
    │   │   └─→ _restoreCache() 部分成功 → 某些頁面資料缺失
    │   └─→ 快速切換聊天室 → pagehide 未觸發持久化（5.5）
    │       └─→ 最近的操作丟失 → 下次回訪看到舊資料
    │
    ├─→ 首次訪問 + Prod 模式的渲染競態（5.17）
    │   └─→ renderAll() 同步執行 → ApiService.getEvents() → _cache 為空
    │       └─→ 首頁活動列表空白 → Cloud 就緒後若無重新渲染機制 → 持續空白
    │
    ├─→ 頁面切換時 page-loader 失敗（5.18）
    │   └─→ CDN 過載 → HTML 片段 fetch 回傳 503 → 無錯誤處理
    │       └─→ 頁面區域空白 + 無用戶提示 → 用戶重整 → 再次打 origin
    │
    └─→ 回訪用戶（佔多數）
        ├─→ localStorage 快取有效 → 秒開 ✅（正常路徑）
        ├─→ localStorage 快取過期/損壞 → 同首次訪問路徑
        └─→ localStorage 配額已滿 → _saveToLS 靜默失敗（5.4）
            └─→ 下次回訪變成「偽首次訪問」→ 觸發全量載入
```

**關鍵結論**：快取系統的每一層失敗都會導致壓力向下一層傳遞。當多層同時失敗時，形成級聯雪崩。Phase E 的目標是加固每一層的失敗處理，打斷這個連鎖。第十次驗證補充了渲染競態（5.17）和 page-loader 錯誤處理缺失（5.18）兩條新的故障路徑。
...
### Step 4：FirebaseService 防重複初始化（E7）
...
### Step 5：onSnapshot 重連加 jitter（E2）
...
### Step 7：page-loader HTTP 錯誤檢查（E9）
...
必做 8 步之間無依賴，可以任意順序施作。
建議按 Step 1→2→3→4→5→6→7→8 的順序（影響從大到小）。
每步獨立 commit + 部署 + 驗證。
```
