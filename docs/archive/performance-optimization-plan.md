...
5 分鐘後造訪，快取就失效，用戶必須重新等待 Firebase 回傳資料。但 SportHub 的資料（活動列表、俱樂部資訊、廣告橫幅等）並不是每分鐘都在變動，短時間內看到稍微舊一點的資料完全可以接受。況且 `onSnapshot` 即時監聽機制會在背景自動將最新資料更新進來，用戶實際上永遠能看到最新資料，只是初始渲染的速度更快。
