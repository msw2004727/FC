...
ToosterX 目前的開機流程分為四個 Phase，**依序執行**：
...
5 分鐘後造訪，快取就失效，用戶必須重新等待 Firebase 回傳資料。但 ToosterX 的資料（活動列表、俱樂部資訊、廣告橫幅等）並不是每分鐘都在變動，短時間內看到稍微舊一點的資料完全可以接受。況且 `onSnapshot` 即時監聽機制會在背景自動將最新資料更新進來，用戶實際上永遠能看到最新資料，只是初始渲染的速度更快。
...
**改動範圍：app.js 開機流程**
**實作狀態：✅ 已完成（Phase 1 非阻塞、Phase 1 完成後補跑 renderAll + _bindPageElements）**
...
7. 以上 1～6 **在 Demo 模式**完整走過一遍（無需登入）
8. 以上 1～6 **在正式模式**完整走過一遍（LINE 登入後測試）
...
#### 最低可用頁面清單（Minimum Viable Pages）

延遲載入腳本分組時，不應只按「檔案大小」切割，應先按「重要性」定義哪些模組是核心必載。即使 Fallback 全部失敗，至少要保證**首頁**與**個人資料頁**可以正常載入。

**核心必載（不可延遲）：**

| 模組 | 負責功能 | 缺失影響 |
|------|---------|---------|
| `app.js` | App 核心物件、初始化流程 | 整個應用無法運作 |
| `api-service.js` | Demo/Prod 資料抽象層 | 所有資料查詢失效 |
| `event-list.js` | 首頁活動列表渲染 | 首頁空白 |
| `event-render.js` | 活動詳情、報名按鈕 | 首頁活動無法互動 |
| `profile-core.js` | 個人資料基礎 | 個人頁空白，登入狀態失效 |
| `banner.js` | 橫幅輪播、公告、贊助商 | 首頁廣告區空白（可接受但不理想） |

**可延後載入（按頁面分組）：**

| 模組群組 | 觸發時機 | 靜默失敗的影響 |
|---------|---------|-------------|
| `scan.js` | 進入掃碼頁 | 掃碼功能不可用，但不影響其他頁面 |
| `leaderboard.js` | 進入個人資料頁（分頁內） | 報名記錄統計不顯示 |
| `achievement.js` | 進入成就頁 | 成就列表不顯示 |
| `team-*.js` | 進入俱樂部頁 | 俱樂部功能不可用 |
| `tournament-*.js` | 進入賽事頁 | 賽事功能不可用 |
| `user-admin-*.js` | 進入後台管理（管理員才能進） | 後台管理不可用 |
| `ad-manage-*.js` | 進入廣告後台 | 廣告管理不可用 |

**實作指導：**

- 方案五的分組設計以「核心必載」群組為優先，確保即使所有延遲腳本都失敗，首頁與個人頁仍可完整運作
- Fallback 第二層的錯誤頁設計也應對照此清單：「核心模組失敗 → 整頁刷新；非核心模組失敗 → 僅該頁提示錯誤」
...
**實作狀態：✅ SW 快取已完成（stale-while-revalidate + loading="lazy"），Cache-Control 待實作**
...
> 直接填入此文件，不需另開獨立記錄檔案。每個階段完成後在下方欄位補填對應數值，可按需自行新增欄位。
...
每個階段實作完成後，依以下清單逐項確認後才算該方案完成。

---

### 通用前置條件（每次量測都要統一）

- [ ] 固定裝置（同一台手機，或 DevTools 固定同一模擬機型）
- [ ] 記錄本次網路環境：行動網路（4G/5G）□ / WiFi □
- [ ] 分兩種情境測試：清除快取後首次造訪 □ / 保留快取重複造訪 □
- [ ] Demo 模式與正式模式各測一次
...
**功能驗證：**
- [ ] 首頁正常載入：橫幅輪播、活動列表、公告皆顯示
- [ ] Demo 模式：瀏覽、報名、取消報名正常
- [ ] 正式模式：LINE 登入成功，個人資料顯示正確

**效果驗證：**
- [ ] 5 分鐘後回來：仍顯示快取內容（不再白屏等待）
- [ ] 10 分鐘後回來：仍顯示快取內容
- [ ] 第二次造訪：圖片瞬間顯示，無明顯 loading 時間

**量測記錄：**
- [ ] LCP / TTI 已填入基準記錄表「階段一後」欄位

---

### 階段二完成確認（方案三：Phase 並行化）

**功能驗證：**
- [ ] 有快取時：開啟後 <200ms 看到內容
- [ ] 無快取時：仍正常顯示，不白屏、不報 console error
- [ ] 所有 tab 切換正常（活動 / 俱樂部 / 個人 / 探索）
- [ ] Demo 模式：瀏覽正常
- [ ] 正式模式：登入流程、個人資料正常
- [ ] Console 無 `null` 相關 TypeError（container null 防護生效）
...
- [ ] 快取未就緒時：首頁出現灰色閃爍佔位，而非空白
- [ ] 資料載入後：骨架屏正確替換為真實內容，不殘留
- [ ] Demo 模式測試一遍
- [ ] 正式模式測試一遍

---

### 階段四完成確認（方案五：JS 分組 / 方案七：資料縮減）

**方案五：**
- [ ] Smoke Test 8 條路徑在 Demo 模式全部通過（無 console error）
- [ ] Smoke Test 8 條路徑在正式模式全部通過
- [ ] Console 無 `ReferenceError: App is not defined`
- [ ] 一般用戶開啟首頁：DevTools Network 確認後台 JS 未被下載

**方案七：**
- [ ] 一般用戶：Firestore Console 確認 `registrations` query 帶 `userId == currentUser`
- [ ] 管理員：`registrations` 全量可見，無缺漏
- [ ] EXP 統計、出席率數字與修改前一致
...
2. **方案五實施前**，必須完成依賴樹分析，並在每次拆分後完整走過 Smoke Test 清單（8 個關鍵路徑，Demo + 正式各一遍）。
...
6. **不要輕易更新 `IMAGE_CACHE_NAME`（`sporthub-images-v1`）**：`CACHE_VERSION` 是每次改版都要動的版本號；`IMAGE_CACHE_NAME` 只有在需要強制清空所有用戶的圖片快取時才應變更（例如修復了 SW 快取邏輯 bug）。誤更新會導致所有用戶的圖片重新從網路下載，速度倒退。靜態資源的 `CACHE_NAME` 同理。
