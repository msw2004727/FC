...
| CF `syncTeamReservationSeatsForUserChange` | `functions/index.js:5869` | user `teamId/teamIds` 變更後修復未開始活動的團隊席位；讀寫 `events/{event}/registrations`，候補遞補時同步 `events/{event}/activityRecords` |
| CF `refundEarlyBirdRegistrationsForCancelledEvent` | `functions/index.js` | 活動狀態改為 cancelled 後讀取 `events/{event}/registrations` 中 `earlyBird == true` 的報名，冪等退回早鳥扣點 |
