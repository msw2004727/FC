...
> **📍 最終決議（2026-04-13）**：經四輪審計 + 7 位專家 7-0 全票投票，**採用極簡方案**（`fetchAttendanceIfMissing` 一次性子集合補查，15 行 / 2 檔案），已實作並部署（commit `28bf3072`）。以下 v5 完整方案保留為歷史參考，記錄「為什麼不用 per-event listener」的完整決策過程。極簡方案勝出原因：舊活動簽到資料是凍結的靜態資料，不需要即時監聽。
