...
- **修復**：
  - 改用真實 DOM 元素（`.h-card-loading-bar` / `.h-card-loading-fill`）置中在縮圖上
  - JS 模擬進度：每 100ms 遞增，快→慢趨近 85%，載入完成時 snap 到 100% 後淡出
  - 縮圖半透明遮罩保留，載入完成時一同淡出
- **教訓**：CSS-only 的 indeterminate 動畫無法呈現「進度」感，需要 JS 驅動 width 才能做到 0→100%

### 2026-03-14 — 首頁活動卡片「載入中」改為底部動畫進度條
- **問題**：卡片上的「載入中」文字膠囊不夠簡約
- **修復**：
  - CSS `::after` 改為卡片底部 3px 進度條，用 `@keyframes hCardLoading` 做 0→75%→0 來回動畫
  - 載入完成時加 `.is-loaded` class，進度條 transition 到 100% 後淡出
  - `_clearHomeEventCardPending` 加入 650ms 完成動畫延遲再移除 class
- **教訓**：indeterminate 進度條 + 完成時 fill-to-100% 的組合比文字提示更直覺

### 2026-03-14 — 首頁活動卡片載入 toast 改為持續顯示 + 5 秒提示重整
- **問題**：點擊首頁活動卡片時的「活動資料載入中」提示 2.5 秒就消失，用戶不知道還在載入中
- **修復**：
  - `_showHomeEventLoadingToast` 改為持續顯示（不走 `showToast` 的 2.5s auto-dismiss）
  - 5 秒後自動將訊息改為「若加載過久請關閉所有分頁並重整瀏覽器」
  - 新增 `_dismissHomeEventLoadingToast()` 在 `openHomeEventDetailFromCard` 的 `finally` 清除
- **教訓**：載入提示的生命週期應與實際載入狀態同步，不應用固定計時器自動消失

### 2026-03-14 — 掃碼簽到：活動選擇加入日期分類篩選
- **問題**：掃碼簽到頁的活動下拉選單為扁平列表，活動多時難以找到目標
- **修復**：
  - 在 `<select>` 上方新增「今日 | 過期 | 未來」分段按鈕（`pages/scan.html`、`css/scan.css`）
  - 新增 `_categorizeScanEvents()` 將活動依日曆日分桶，各 bucket 獨立排序（`js/modules/scan.js`）
  - 非 Admin 移除日期限制（改由 UI 篩選），僅保留擁有者/委派者過濾
  - 預設「今日」tab，若今日無活動自動切到第一個有活動的 tab
  - 單一活動時自動選取；預設活動模式（`_scanPresetEventId`）時隱藏篩選列
  - 選項文字加上活動類型前綴如 `[PLAY]`、`[友誼]`
- **教訓**：當列表可能很長時，分類篩選比搜尋框更適合行動端操作
