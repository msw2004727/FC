...
- **Tests**: Added a source contract test for the fast-loading shell and scoped light loading panel style.
...
- **驗收**：JS 語法檢查覆蓋 `app.js`、`navigation.js`、`script-loader.js`、`firebase-service.js`、活動/俱樂部/賽事列表模組與注入腳本；公開快照不標記 `lazyLoaded=true`，所以 Firestore 仍會背景刷新，報名/取消/管理寫入不吃快照。
...
### 2026-05-05 活動預設封面自動上傳 [ux]
- **問題**: 新增活動未上傳圖片時會留下空封面，活動卡片與詳情頁缺少一致的視覺預設。
...
- **統計**: 活動卡瀏覽量顯示改為記錄值 + 500 的起始展示值，無瀏覽資料時也從 500 起跳。
- **版號**: 左側抽屜版號與下載 APP 按鈕拉開距離，避免貼太近。
- **驗證**: 更新 `home-dashboard-render.test.js` 覆蓋 0 活動不渲染、「查看更多」入口與瀏覽量 +500。

### 2026-05-06 首頁目前資訊文案調整 [ux]
- **調整**: 首頁統計區補回「目前資訊」標題，三張卡片文案改為「已開放活動」「已成立俱樂部」「正舉辦賽事」。
- **視覺**: 統計數字改用較寬的黑體數字字型設定，維持卡片尺寸與原本色彩區分。
- **驗證**: 更新首頁 dashboard render 測試覆蓋新文案，保留活動瀏覽量 +500 與既有連結行為。
...
### 2026-05-09 Activity Social Links Threads Block Icon [ux]
...
### 2026-05-11 History API Dual Route Phase 0-3 [feature]
- **Issue**: The app only had hash/query deep links, so clean URLs such as `/activities` or `/events/{id}` could not be refreshed safely without explicit hosting fallback and boot translation.
...
### 2026-05-12 PM Audit Overflow Fix [bugfix]
- **Issue**: The super_admin chat audit panel could overflow horizontally because long UIDs, conversation IDs, timestamps, and log rows were not fully constrained inside the admin card layout.
- **Fix**: Added width/min-width guards to the chat audit panel, cards, result lists, user/thread rows, conversation rows, and log rows. Desktop log rows now keep the action/time columns stable and ellipsize long UID text; narrow screens wrap the log content safely.
- **Validation**: Added source contract coverage for the PM audit overflow CSS and ran the targeted private message unit test.
...
### 2026-05-13 Club Detail Action Bar and Settings Polish [ux]
...
### 2026-05-14 Changelog Commit Time Compatibility [feature]
- **Issue**: The changelog sync stored only date-level string entries, so `/changelog/` could not show commit times and would break if Firestore entries were changed to objects without renderer support.
- **Fix**: Updated `scripts/sync-changelog.js` to read commit ISO time from Git, derive Asia/Taipei date and `h:mmam/pm` display time, and write changelog entries as `{ message, time, committedAt }`. Updated the Firestore REST serializer to support arrays of map objects. Updated `/changelog/` rendering to accept both legacy string entries and new object entries.
- **Validation**: Ran `node -c scripts/sync-changelog.js`, parsed the changelog inline script with `new Function`, and smoke-tested `getGitLog()` plus Firestore field conversion with a real latest commit sample. Then resynced production changelog data for 2026-02 through 2026-05, covering 2,811 entries.

### 2026-05-14 Event Registration Failure Diagnostics [bugfix]
...
### 2026-05-16 Activity Display Cache Window [performance]
- **Issue**: Activity/calendar/detail pages had realtime refresh, but cold re-entry after the localStorage cache was older than the fresh window could still spend more time on Firestore before showing useful cached shell data. The stale cache fallback also had no hard display cap.
...
- **修復**：條款預設保持未勾選；按「同意並送出」未勾選時改用 toast 顯示 `請先勾選同意服務條款與隱私權政策。`；按「稍後填寫」即使未勾選也可關閉彈窗。E2E 關閉 helper 也改走未勾選的「稍後填寫」路徑，避免離線煙霧測試等待條款同意寫入。
- **教訓**：法律同意提示要跟實際送出行為綁定，暫緩填寫流程應維持低阻力；測試 helper 不能為了關窗強制製造使用者未做出的同意行為。

### 2026-05-19 — Toast Width And Profile Required Fields [bugfix]
- **問題**：toast 在部分手機環境看起來太窄，且首次完善個人資料必填欄位提示顯示在表單內，使用者可能沒注意到。
...
### 2026-05-19 — Profile Later Button No Consent Save [bugfix]
- **問題**：首次完善個人資料彈窗按「稍後填寫」時，若使用者剛好有勾選條款，仍會寫入條款同意紀錄。
- **原因**：「稍後填寫」曾被設計成已勾選時補存同意，但此按鈕語意其實是暫不同意、暫不送出。
...
### 2026-05-19 — Page Back Buttons SVG Icon [ux]
- **問題**：主 App 各頁左上返回鍵仍以 `‹` / `←` 文字符號呈現，視覺粗細與對齊不一致。
...
### 2026-05-21 — Ops LTV AI Prompt Pack [feature]
- **問題**：營運報表只能匯出每日 CSV，使用者要交給其他 AI 分析時，缺少可一次複製或下載完整口徑、總覽、留存、轉化與每日資料的 AI 資料包。
- **原因**：`ops-report.html` 只提供畫面渲染與 CSV 匯出，沒有把目前 `report` 物件、資料口徑與 AI 分析限制打包成可貼上的文字或可匯入附件的 JSON。
- **修復**：在頁首登入狀態左側新增「複製AI分析提示詞」與「下載AI分析JSON」兩個按鈕；報表載入後可複製包含完整 JSON、每日序列 CSV、資料口徑與「只能依據提示詞資料分析、不得腦補外部數字」限制的提示詞，也可下載同內容的結構化 JSON 資料包；CSV 匯出改共用同一個序列產生器。
- **教訓**：給 AI 的營運資料包不只要有數值，也要把口徑與禁止外推的規則一起放進提示詞與 JSON，避免下游分析把缺資料當成可推測結論。

### 2026-05-21 — Club Education Session UI Polish [ux]
- **問題**：課堂卡片在窄螢幕上的編輯/刪除圖示辨識度太接近；方案學員備註會擠壓斷行且編輯入口離暱稱太遠；編輯課堂的範本區塊佔用過多空間；本堂學員全選/清空缺少名單脈絡。
...
### 2026-05-22 — Club Detail V2 Polish Follow-up [ux]
- **問題**：俱樂部資訊卡的暱稱膠囊在雙欄資訊內行高跑位；淺色課程封面套白色遮罩後，課堂標籤顏色過淡；封面積分與評級字級不一致且沒有落在招收列右下；無戰績時勝率圓環的 `-` 與標籤垂直位置不穩。
- **原因**：資訊卡沿用全站 user capsule 的絕對定位 Lv badge；課程標籤仍套深色封面用的透明白字；積分 badge 仍浮在圖片層；勝率圓環內文用 grid 疊放，文字與標籤容易互相壓位。
- **修復**：新增 v2 資訊卡 scoped compact capsule，將 Lv badge 改為膠囊內行內元素；淺色課程封面標籤改為高對比底色與邊框；將積分/評級移入招收列右下且統一 18px；勝率圓環內文改為 flex column 垂直置中。
- **教訓**：全站通用小元件放進密集資訊卡時要有局部變體，不應直接繼承絕對定位與外距；同一張圖片在不同遮罩主題下，標籤色彩也要一起切換。

### 2026-05-22 — Club Detail V2 Info Card Cleanup [ux]
- **問題**：新版俱樂部資訊卡仍重複封面已有的運動、地區、成立與教學標籤，導致經理/領隊/聯繫方式在窄版卡片中擠壓跑位；底部快速操作 FAB 會在導覽列上方露出不明綠色按鈕；成員分頁的橘色總數徽章容易被誤解為待審或新成員數；頭像與封面上傳按鈕過大且深色主題不夠協調。
- **原因**：v2 資訊卡仍沿用 demo 階段的完整 fact grid；FAB 是早期快速入口但沒有實際必要；分頁 badge 沒區分統計數與通知數；媒體編輯按鈕沿用較大的方角浮層樣式。
- **修復**：資訊卡改只顯示經理、領隊與聯繫方式，聯繫按鈕補齊 v2 自身圓形與對齊樣式；移除 FAB DOM 與樣式；成員分頁不再顯示總數徽章，快速導覽改標示「成員總數」；上傳按鈕縮小約 25%、改圓形並補上深色主題色。
- **教訓**：封面與資訊卡要明確分工，重複資訊會在窄版造成跑位；badge 只有在語意像通知或待處理事項時才適合用醒目色，純統計數應放在統計區或清楚標籤中。

### 2026-05-22 — Club Detail V2 Share, Count, and Back Fixes [bugfix]
- **問題**：俱樂部資訊卡長暱稱會撐開膠囊；課程方案數把已結束課程也算入；從其他頁返回俱樂部詳細頁時舊版封面容器會露出大片空白；分享文字帶入地區與人數且複製連結仍走 Mini App；新增的教育學員沒有同步進入成員統計。
