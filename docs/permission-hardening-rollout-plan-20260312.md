...
1. **Step 1**：建立權限真相表與能力矩陣（基礎）
2. **Step 2**：驗證並強化權限管理後台保護（快速勝利，降低升權風險）
3. **Step 3**：用戶管理 capability 拆分（作為範本）
4. **Step 4**：驗證並強化 Rules 敏感欄位保護（配合 Step 3 成果）
5. **Step 5**：其他功能 capability 化（依 Step 3 範本擴張）
6. **Step 6**：角色切換即時生效閉環
7. **Step 7**：清理 `isAdmin` 舊欄位殘留依賴
8. **Step 8**：測試與第三方驗證
...
| Step 5 其他功能 capability | 高 | 逐功能盤點 + 三層同步改造 |
