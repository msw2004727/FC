...
- 一般 user 建立/編輯自己的活動時，只有在 `roleActivityCapabilities/user` 開啟 `user.activity.addons_use` 後才能寫入進階功能欄位；未開啟時前端顯示 `如需更多功能請聯繫官方Line@`，Firestore Rules 也會拒絕 fee、teamOnly、gender restriction、private event、teamSplit、socialLinks 等進階功能欄位。
