...
以下 10 頁目前**沒有 SWR 策略定義**，預設走 `fresh-first`（必須等 Firebase 就緒才渲染），不享受快取優先。需在 `js/config.js` 的 `PAGE_STRATEGY`、`PAGE_DATA_CONTRACT` 和 `js/firebase-service.js` 的 `_collectionPageMap` 補上。

#### 教育俱樂部頁面（6 頁）

| 頁面 ID | 建議策略 | 需要的集合 | 說明 |
|---------|---------|-----------|------|
| `page-edu-groups` | stale-first | teams, educationGroups | 學員分組列表 |
| `page-edu-students` | stale-first | teams, students | 學員名冊 |
| `page-edu-checkin` | stale-first | attendanceRecords, events | 課堂簽到 |
| `page-edu-calendar` | stale-first | events | 課程行事曆 |
| `page-edu-course-plan` | stale-first | courses | 課程方案 |
| `page-edu-course-enrollment` | stale-first | enrollments | 報名管理 |

#### 遊戲頁面（2 頁）

| 頁面 ID | 建議策略 | 需要的集合 | 說明 |
|---------|---------|-----------|------|
| `page-game` | stale-first | gameConfigs | 射門遊戲 |
| `page-kick-game` | stale-first | gameConfigs | 踢球遊戲 |

#### 成就 / 稱號頁面（2 頁）

| 頁面 ID | 建議策略 | 需要的集合 | 說明 |
|---------|---------|-----------|------|
| `page-achievements` | stale-first | achievements, badges | 成就列表 |
| `page-titles` | stale-first | titles | 稱號列表 |

#### 覆蓋率變化

| | 補齊前 | 補齊後 |
|---|---|---|
| 有 SWR 策略的頁面 | 25 / 35（71%） | **35 / 35（100%）** |
| 教育俱樂部 | 0 / 6 | 6 / 6 |
| 遊戲 | 0 / 2 | 2 / 2 |
| 成就/稱號 | 0 / 2 | 2 / 2 |

> ※ 工具頁（QR 產生、名片分享等 5 頁）非用戶主動導航頁面，不需 SWR 策略。
...
- 10 頁 SWR 策略 → `js/config.js` + `js/firebase-service.js`（設定補齊）
...
在 `js/config.js` 的 `PAGE_STRATEGY` 和 `PAGE_DATA_CONTRACT` 補上教育俱樂部（6 頁）、遊戲（2 頁）、成就/稱號（2 頁）的策略定義。在 `js/firebase-service.js` 的 `_collectionPageMap` 補上對應集合。

此步驟為純設定新增，**不修改任何邏輯程式碼**，風險最低。需在 Step 5 之後執行，確保導航 cloud-gate 已放寬，新策略才能生效。

完成後全站 35 個用戶導航頁面達到 **100% SWR 覆蓋**。
...
| Step 6：補齊 10 頁 SWR 策略 | 30 分鐘 | config.js + firebase-service.js 設定新增 |
...
7. **全站 100% 覆蓋** — 補齊教育俱樂部（6 頁）、遊戲（2 頁）、成就/稱號（2 頁）的 SWR 策略後，35 個用戶導航頁面全部享受快取優先
