...
const InvPermissions = {
  ...
  CATALOG: [
    ...
    { group: '庫存管理', items: [
      ...
      { code: 'inventory.split', label: '拆分庫存', desc: '將商品部分庫存拆分到其他頁籤（商品/活動/器材/其他），拆分後共用條碼、各自獨立管理。可隨時合併回原商品。' },
      { code: 'inventory.transfer', label: '跨倉庫調撥', desc: '將商品庫存從當前倉庫調撥至其他倉庫，支援全部或部分轉移。' },
      { code: 'inventory.see_cost', label: '查看進貨價', desc: '可看到商品的進貨價（成本價）與利潤相關數據。關閉後進貨價顯示為 ***，但不影響其他功能。' },
    ]},
    { group: '銷售紀錄', items: [
    ...
    { group: '設定', items: [
      ...
      { code: 'settings.group_tabs', label: '頁籤管理', desc: '修改庫存頁面的群組頁籤顯示名稱（商品/活動/器材/其他）。僅修改顯示名稱，不影響商品資料。' },
    ]},
    { group: '導航', items: [
  ...
  DEFAULTS: {
    manager: {
      ...
      'inventory.split': true, 'inventory.transfer': true, 'inventory.see_cost': true,
    ...
    leader: {
      ...
      'inventory.split': true, 'inventory.transfer': false, 'inventory.see_cost': false,
      'transactions.view': true, 'transactions.export': true, 'transactions.reports': true,
      'stocktake.start': true, 'stocktake.scan': true, 'stocktake.apply': true, 'stocktake.history': true,
    ...
    staff: {
      ...
      'inventory.split': false, 'inventory.transfer': false, 'inventory.see_cost': false,
