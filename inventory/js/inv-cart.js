...
const InvCart = {
   ...
   */
  add(product) {
    var existing = this.items.find(function (it) { return it.barcode === product.barcode; });
    if (existing) {
      existing.quantity++;
    } else {
      this.items.push({
   ...
   */
  remove(barcode) {
    this.items = this.items.filter(function (it) { return it.barcode !== barcode; });
    this._onUpdate();
  },
  ...
  updateQuantity(barcode, qty) {
    ...
    var item = this.items.find(function (it) { return it.barcode === barcode; });
    if (item) {
      item.quantity = qty;
      this._onUpdate();
    }
  ...
  renderCartList(containerId) {
    ...
    for (var i = 0; i < this.items.length; i++) {
      ...
      var bc = esc(it.barcode);

      html +=
        '<div class="inv-cart-item">' +
          '<div class="name">' + esc(it.name) + '</div>' +
          '<div class="inv-qty-btns">' +
            '<button onclick="InvCart.updateQuantity(\'' + bc + '\',' + (it.quantity - 1) + ')">-</button>' +
          ...
          '<button style="border:none;background:none;color:var(--inv-danger);font-size:18px;cursor:pointer;padding:4px 8px;" ' +
            'onclick="InvCart.remove(\'' + bc + '\')">✕</button>' +
        '</div>';
    }
