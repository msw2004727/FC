...
describe('team detail club activity section', () => {
  ...
  test('shows education pending tab badge for staff and non staff own pending students', () => {
    const app = makeApp([]);
    Object.assign(app, {
      isEduClubStaff: () => false,
      getEduStudents: () => [
    ...
    Object.assign(app, {
      isEduClubStaff: () => true,
      getEduStudents: () => [
        { id: 'pending-a', enrollStatus: 'pending', selfUid: 'student-a' },
        { id: 'pending-b', enrollStatus: 'pending', selfUid: 'student-b' },
        { id: 'active-c', enrollStatus: 'active', selfUid: 'student-c' },
      ],
    });

    const staffHtml = app._buildTeamEducationSection({ id: 'teamA' });
  ...
  test('team member list merges players, staff, students, and member data into an integrated list', () => {
    ...
    expect(app._buildTeamDetailOverview(team, 0, 0)).toMatch(/td-overview-coach[\s\S]*<strong>1<\/strong>/);
    const multiIdentityCoachTeam = { ...team, coachUids: ['coach', 'captain'] };
    expect(app._getTeamDetailCoachCount(multiIdentityCoachTeam)).toBe(2);
    expect(app._buildTeamDetailOverview(multiIdentityCoachTeam, 0, 0)).toMatch(/td-overview-coach[\s\S]*<strong>2<\/strong>/);
    ...
    expect(html).toContain('td-member-management-panel');
    ...
    expect(html).toContain('td-member-roster-code');
    expect(html).toContain('td-member-roster-no');
    expect(html).toContain('td-member-roster-pos');
    expect(html.indexOf('td-member-roster-code')).toBeLessThan(html.indexOf('td-member-name-cell'));
    ...
    expect(manageHtml).not.toContain('App.editTeamMemberNote');
    expect(manageHtml).not.toContain('App.editTeamMemberMatchData');
    expect(manageHtml).toContain('td-member-list-note-hint');
    expect(courseHtml).not.toContain('td-member-note-edit-btn');
    expect(courseHtml).not.toContain('td-member-course-edit-btn');
    expect(courseHtml).not.toContain('App.editTeamMemberNote');
    expect(activityEditHtml).toContain('td-member-note-edit-btn');
    expect(activityEditHtml).toContain('td-member-course-edit-btn');
    expect(activityEditHtml).toContain('td-member-match-edit-btn');
    expect(activityEditHtml).toContain('App.editTeamMemberNote');
    expect(activityEditHtml).toContain('App.editTeamMemberMatchData');
    ...
    expect(matchHtml).not.toContain('td-member-match-edit-btn');
  ...
  test('member management renders promote and demote columns only with eligible arrow actions', () => {
    ...
    const managerRow = {
      key: 'uid:manager',
      uid: 'manager',
      user: { uid: 'manager', teamIds: ['teamA'] },
      isMember: true,
      roles: new Set(['\u7403\u7d93']),
    };
  ...
  test('role level action promotes member to coach and refreshes the member card', async () => {
    ...
    const roleChange = { uid: 'member', oldRole: 'user', newRole: 'coach', userName: 'Amy' };
    const app = {};
    loadTeamDetailCore(app, null, {
      ApiService: {
        ...
        updateTeamAwait,
        _recalcUserRole: jest.fn().mockReturnValue(roleChange),
        _writeOpLog: jest.fn(),
      },
      FirebaseService: {
        _ensureAuth: jest.fn().mockResolvedValue(true),
    ...
    Object.assign(app, {
      ...
      _deliverMessageWithLinePush: jest.fn(),
      _refreshTeamMembersCardFromCache: jest.fn().mockReturnValue(true),
      _refreshTeamDetailMembers: jest.fn().mockResolvedValue(),
      showToast: jest.fn(),
    });
  ...
  test('role level actions are hidden and blocked for non-manager staff', async () => {
    ...
    const app = {
      _canQuickPromoteTeamMember: () => false,
      _isUserInTeam: () => true,
    };
    loadTeamDetailRender(app);
