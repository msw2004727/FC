...
Object.assign(App, {
  _renderEduCoursePlanFormV2(ctx) {
    ...
    const perSessionBillingChecked = (typeof this._isCoursePlanPerSessionBilling === 'function'
      ? this._isCoursePlanPerSessionBilling(plan)
      : plan?.perSessionBilling === true) ? ' checked' : '';
    ...
    container.innerHTML = '<div class="ce-form edu-cp-form-v2">' +
      ...
      '<section class="edu-cp-section edu-cp-core">' +
        ...
        '</div>' +
        '<div class="edu-cp-toggle-row edu-cp-billing-toggle-row">' +
          '<div><label>隨堂單獨收費</label><small>開啟後不使用整期繳費判定，課堂名單改顯示簽到次數與出席率。</small></div>' +
          '<label class="toggle-switch"><input type="checkbox" id="edu-cp-per-session-billing"' + perSessionBillingChecked + '><span class="toggle-slider"></span></label>' +
        '</div>' +
