    ...
    return `<div class="tc-match-detail-timeline">
      ${events.map(ev => {
        ...
        return `<div class="tc-match-detail-event tc-match-detail-event-${escapeHTML(safeType)}">
          <span class="tc-match-detail-event-icon" aria-hidden="true">${this._getTournamentMatchEventIcon(type)}</span>
  ...
  _renderTournamentMatchEventsSummaryHtml(match, home, away) {
    ...
    const shown = events.slice(0, 6).map(ev => {
      ...
      return `<span class="tc-match-event-card tc-match-event-${escapeHTML(safeType)}" title="${escapeHTML([meta.title, meta.body].filter(Boolean).join(' · '))}">
        <span class="tc-match-event-icon" aria-hidden="true">${this._getTournamentMatchEventIcon(type)}</span>
        <span class="tc-match-event-copy">
          <strong>${escapeHTML(meta.title)}</strong>
          <small>${escapeHTML(meta.body)}</small>
        </span>
      </span>`;
  ...
  _ensureTournamentMatchDetailModal() {
    ...
    overlay.innerHTML = `
      <div class="modal tc-match-info-modal" id="tournament-match-detail-modal">
        <div class="modal-header tc-match-modal-closebar">
