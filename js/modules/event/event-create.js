  ...
  _ceInfoData: {
    ...
    teamSplit: {
      ...
      body: '開啟後系統會在報名流程中加入<b>隊伍分配機制</b>，讓參加者到場前就知道自己穿什麼顏色背心。'
        ...
        + '勾選後系統自動計算每隊人數上限，避免隊伍人數嚴重失衡。'
        + '<div style="margin:.4rem 0;padding:.4rem .6rem;background:var(--bg-elevated);border-radius:6px;border-left:3px solid var(--accent)">'
        + '<div style="font-size:.75rem;font-weight:600;margin-bottom:.2rem">計算方式</div>'
        + '<div style="font-size:.75rem;color:var(--text-secondary)">每隊上限 = 活動人數上限 ÷ 隊伍數（無條件進位）</div>'
        + '<div style="font-size:.75rem;font-weight:600;margin:.3rem 0 .2rem">範例</div>'
        + '<div style="font-size:.75rem;color:var(--text-secondary)">• 20 人活動 ÷ 2 隊 → 每隊上限 <b>10 人</b><br>'
        + '• 20 人活動 ÷ 3 隊 → 每隊上限 <b>7 人</b>（20÷3=6.67 進位）<br>'
        + '• 15 人活動 ÷ 4 隊 → 每隊上限 <b>4 人</b></div>'
        + '<div style="font-size:.72rem;color:var(--text-muted);margin-top:.2rem">自選模式：滿隊後無法再選該隊<br>隨機模式：系統自動平衡分配</div>'
        + '</div>'
