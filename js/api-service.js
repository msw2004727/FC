/* ================================================
   ...
   === Function Index ===
   L207:  _src(key) — 取得快取資料來源
   L212:  _findById(key, id) — 依 ID 查找
   L225:  _create / L272: _update — 通用 CRUD
   L356:  getActiveEvents / getHotEvents
   L425:  getFriendlyTournamentRecord
   L436:  createTournament / updateTournament / L502: deleteTournament
   L669:  getUserCorrection
   L736:  getUserRole
   L758:  getRolePermissions / L786: getRolePermissionDefaults — 權限查詢
   L802:  getRegistrationsByUser / getRegistrationsByEvent
   L841:  getActivityRecords / addActivityRecord / removeActivityRecord
   L872:  fetchAttendanceRecordsForEvent / getAttendanceRecords / getUserAttendanceRecords
   ================================================
   ================================================ */
...
const ApiService = {
  ...
  async updateTournamentEntryMemberProfile(tournamentId, teamId, memberUid, profile = {}) {
    const safeNumber = String(profile.jerseyNumber || '').trim();
    if (!/^\d{0,3}$/.test(safeNumber)) throw new Error('INVALID_TOURNAMENT_JERSEY_NUMBER');
    if (!(await FirebaseService.ensureAuthReadyForWrite())) throw new Error('AUTH_NOT_READY');
    return await FirebaseService.updateTournamentEntryMemberProfile(tournamentId, teamId, memberUid, {
      jerseyNumber: safeNumber,
      position: String(profile.position || '').trim().slice(0, 20),
      note: String(profile.note || '').trim().slice(0, 30),
    });
  },
