...
const CACHE_VERSION = '0.20260617';
...
const ACTIVITY_MAP_CONFIG = {
  ...
  googleTileFallbackMs: 7000,
  googleLayoutSettleDelaysMs: [120, 450],
};
...
function isActivityMapLocationPickerEnabled() {
  if (isActivityMapDiagnosticOverride()) return true;
  const flags = (typeof FirebaseService !== 'undefined' && typeof FirebaseService.getCachedDoc === 'function')
    ? FirebaseService.getCachedDoc('siteConfig', 'featureFlags')
    : null;
...
const ACTIVITY_DETAIL_OPTIMIZATION_FLAGS = {
  ...
  detailAttendanceOnDemand: false,
};
...
const PAGE_DATA_CONTRACT = {
  ...
  'page-team-detail':        { required: [], optional: ['teams', 'events', 'tournaments', 'roleActivityCapabilities'], realtime: [] },
...
// 2026-04-25：自製 HTML/SVG 圖示對照表（優先於 SPORT_ICON_EMOJI 在網頁 UI 使用）
...
const SPORT_ICON_SVG_HTML = {
  football: '<img src="./img/Socce.png" alt="" width="20" height="20" loading="lazy" decoding="async">',
...
const DRAWER_MENUS = [
  ...
  { icon: '', label: '俱樂部管理', i18nKey: 'drawer.teamManage', page: 'page-team-manage', minRole: 'captain', permissionCode: 'team.manage.entry' },
  { divider: true, minRole: 'admin' },
  { sectionLabel: '後台管理', i18nKey: 'drawer.backendManage', minRole: 'admin' },
  { icon: '', label: '小遊戲管理', i18nKey: 'admin.gameManage', page: 'page-admin-games', minRole: 'admin', permissionCode: 'admin.games.entry' },
  { icon: '', label: '用戶管理', i18nKey: 'admin.userManage', page: 'page-admin-users', minRole: 'admin', permissionCode: 'admin.users.entry' },
  { icon: '', label: '首頁管理', i18nKey: 'admin.adManage', page: 'page-admin-banners', minRole: 'admin', permissionCode: 'admin.banners.entry' },
  ...
  { icon: '', label: '推播通知設定', i18nKey: 'admin.notifSettings', page: 'page-admin-notif', minRole: 'super_admin', permissionCode: 'admin.notif.entry' },
...
const ROLE_ACTIVITY_CAPABILITY_ITEMS = Object.freeze([
  ...
  },
  {
    ...
    name: '使用進階功能（加值服務）',
    description: '預設關閉。開啟後一般 user 才能使用收費、隊伍限定、性別限定、私密活動、分隊、社群連結、早鳥報名與 GPS 地圖座標等進階功能（加值服務）開關。',
