...
const EDU_COURSE_ROSTER_SYNC_FUNCTION_OPTIONS = Object.freeze({
  region: "asia-east1",
  timeoutSeconds: 60,
  memory: "256MiB",
  maxInstances: 2,
});
const EDU_COURSE_ROSTER_SYNC_TRIGGER_OPTIONS = Object.freeze({
  ...EDU_COURSE_ROSTER_SYNC_FUNCTION_OPTIONS,
  retry: true,
});
const EDU_COURSE_ROSTER_SYNC_MAX_SESSIONS = 160;
...
const OG_ASSET_VERSION = "20260707";
 ...
 */
function createEduCourseHttpsError(code, message, details = undefined) {
  const map = {
    PLAN_NOT_FOUND: "not-found",
    PLAN_INACTIVE: "failed-precondition",
    SIGNUP_CLOSED: "failed-precondition",
    PLAN_ENDED: "failed-precondition",
    PLAN_HIDDEN: "permission-denied",
    NO_STUDENTS: "invalid-argument",
    NO_ELIGIBLE_STUDENTS: "failed-precondition",
    STUDENT_NOT_FOUND: "not-found",
    STUDENT_FORBIDDEN: "permission-denied",
    STUDENT_INACTIVE: "failed-precondition",
    ALREADY_ENROLLED: "already-exists",
  ...
  return new HttpsError(map[code] || "failed-precondition", message || code, details || { code });
}

function normalizeEduCourseRequestIds(data = {}) {
  const teamId = sanitizeStr(data.teamId, 100);
  const planId = sanitizeStr(data.planId, 100);
...
function hasEduCourseMigrationAccess(access) {
  return !!access && (
    access.isSuperAdmin
    || access.hasPermission("team.manage_all")
    || access.hasPermission("admin.repair.data_sync")
  );
}
...
async function loadCourseStudentsForDecision(tx, teamRef) {
  const snap = await tx.get(teamRef.collection("students"));
...
async function loadCourseEnrollmentsForDecision(tx, planRef) {
  const snap = await tx.get(planRef.collection("enrollments"));
  return snap.docs.map((doc) => ({ id: doc.id, _docId: doc.id, ...(doc.data() || {}) }));
}
function normalizeEduCourseRosterStudentIds(studentIds = []) {
  return Array.from(new Set((Array.isArray(studentIds) ? studentIds : [])
    .map((value) => sanitizeStr(value, 100))
    .filter(Boolean)));
}
function sameEduCourseRosterStudentIds(left = [], right = []) {
  const a = normalizeEduCourseRosterStudentIds(left);
  const b = normalizeEduCourseRosterStudentIds(right);
  return a.length === b.length && a.every((value, index) => value === b[index]);
}
function sameEduCourseStringSet(left = [], right = []) {
  const a = normalizeEduCourseRosterStudentIds(left).sort();
  const b = normalizeEduCourseRosterStudentIds(right).sort();
  return a.length === b.length && a.every((value, index) => value === b[index]);
}
function buildEduCourseEffectiveRosterStudentIds({ plan = {}, enrollments = [], students = [], migrationCompleted = false } = {}) {
  const studentsById = new Map();
  (Array.isArray(students) ? students : []).forEach((student) => {
    const studentId = sanitizeStr(student?.id || student?._docId, 100);
    if (studentId) studentsById.set(studentId, student);
  });
  const enrollmentNameByStudentId = new Map();
  (Array.isArray(enrollments) ? enrollments : []).forEach((enrollment) => {
    const studentId = sanitizeStr(enrollment?.studentId, 100);
    const studentName = sanitizeStr(enrollment?.studentName, 100);
    if (studentId && studentName && !enrollmentNameByStudentId.has(studentId)) {
      enrollmentNameByStudentId.set(studentId, studentName);
    }
  });
  const ids = normalizeEduCourseRosterStudentIds(Array.from(getApprovedStudentIdSet({
    plan,
  ...
  return ids.sort((leftId, rightId) => {
    const leftStudent = studentsById.get(leftId) || {};
    const rightStudent = studentsById.get(rightId) || {};
    const leftName = firstSanitizedString([
      leftStudent.name,
      leftStudent.displayName,
      leftStudent.studentName,
      enrollmentNameByStudentId.get(leftId),
      leftId,
    ], 100);
    const rightName = firstSanitizedString([
      rightStudent.name,
      rightStudent.displayName,
      rightStudent.studentName,
      enrollmentNameByStudentId.get(rightId),
      rightId,
    ], 100);
    const nameCompare = leftName.localeCompare(rightName, "zh-Hant");
    return nameCompare || leftId.localeCompare(rightId);
  });
}
function isEduCourseAutoManagedSession(session = {}) {
  const sessionId = sanitizeStr(session.id || session._docId, 100);
  const autoSource = sanitizeStr(session.autoSource, 80);
  return session.autoGenerated === true
    || autoSource === "session-plan"
    || autoSource === "weekly-plan"
    || /^auto_(session|weekly)_/.test(sessionId);
}
function getEduCourseSessionStartMs(session = {}) {
  const date = sanitizeStr(session.date, 20);
  if (!date) return null;
  const startTime = firstSanitizedString([session.startTime], 20) || "00:00";
  const endTime = firstSanitizedString([session.endTime], 20);
  const eventDate = normalizeCourseLessonEventDateText(date, startTime, endTime);
  const startDate = parseEventStartDateInTaipei(eventDate);
...
function getEduCourseSessionRosterSyncSkipReason(session = {}, nowMs = Date.now()) {
  ...
  if (startMs === null) return "missing_session_time";
  if (startMs <= nowMs) return "started_or_past_session";
  return "";
}

function hasEduCourseEnrollmentRosterRelevantChange(beforeData, afterData) {
  const beforeStatus = sanitizeStr(beforeData?.status, 32);
  const afterStatus = sanitizeStr(afterData?.status, 32);
  if (!beforeData || !afterData) return beforeStatus === "approved" || afterStatus === "approved";
  return sanitizeStr(beforeData.studentId, 100) !== sanitizeStr(afterData.studentId, 100)
    || beforeStatus !== afterStatus;
}
function getEduCourseStudentGroupIds(data = {}) {
  const ids = Array.isArray(data?.groupIds) ? data.groupIds : [];
  const groupId = sanitizeStr(data?.groupId, 100);
  return normalizeEduCourseRosterStudentIds(groupId ? [...ids, groupId] : ids);
}
function hasEduCourseStudentRosterRelevantChange(beforeData, afterData) {
  const beforeGroups = getEduCourseStudentGroupIds(beforeData || {});
  const afterGroups = getEduCourseStudentGroupIds(afterData || {});
  if (!beforeGroups.length && !afterGroups.length) return false;
  return !sameEduCourseStringSet(beforeGroups, afterGroups)
    || sanitizeStr(beforeData?.enrollStatus, 32) !== sanitizeStr(afterData?.enrollStatus, 32);
}
function hasEduCoursePlanRosterRelevantChange(beforeData, afterData) {
  if (!beforeData || !afterData) {
    return !!(sanitizeStr(beforeData?.groupId, 100) || sanitizeStr(afterData?.groupId, 100));
  }
  return sanitizeStr(beforeData.groupId, 100) !== sanitizeStr(afterData.groupId, 100)
    || sanitizeStr(beforeData.planType, 32) !== sanitizeStr(afterData.planType, 32);
}
async function syncEduCoursePlanAutoSessionRostersInternal({
  teamId,
  planId,
  source = "course_roster_sync",
  nowMs = Date.now(),
  maxSessions = EDU_COURSE_ROSTER_SYNC_MAX_SESSIONS,
} = {}) {
  const safeTeamId = sanitizeStr(teamId, 100);
  const safePlanId = sanitizeStr(planId, 100);
  const safeSource = sanitizeStr(source, 80) || "course_roster_sync";
  if (!safeTeamId || !safePlanId) {
    return { success: false, reason: "missing_context", teamId: safeTeamId, planId: safePlanId };
  }
  const teamDoc = await getTeamDocByTeamId(safeTeamId);
  if (!teamDoc) return { success: false, reason: "team_not_found", teamId: safeTeamId, planId: safePlanId };
  const planRef = teamDoc.ref.collection("coursePlans").doc(safePlanId);
  const safeMaxSessions = Math.max(1, Math.min(
    EDU_COURSE_ROSTER_SYNC_MAX_SESSIONS,
    normalizeNonNegativeInteger(maxSessions, EDU_COURSE_ROSTER_SYNC_MAX_SESSIONS)
  ));

  const syncResult = await db.runTransaction(async (tx) => {
    const [planSnap, enrollSnap, studentsSnap, sessionsSnap, linksSnap, migrationCompleted] = await Promise.all([
      tx.get(planRef),
      tx.get(planRef.collection("enrollments")),
      tx.get(teamDoc.ref.collection("students")),
      tx.get(planRef.collection("sessions")),
      tx.get(planRef.collection("sessionEventLinks")),
      loadEduAutoMigrationCompletedInTransaction(tx),
    ]);
    if (!planSnap.exists) {
      return { success: true, skipped: true, reason: "plan_not_found", teamId: safeTeamId, planId: safePlanId };
    }
    const plan = { id: planSnap.id, _docId: planSnap.id, ...(planSnap.data() || {}) };
    const planType = sanitizeStr(plan.planType, 32);
    if (planType !== "weekly" && planType !== "session") {
      return { success: true, skipped: true, reason: "non_auto_course_plan", teamId: safeTeamId, planId: safePlanId, planType };
    }
    ...
    const rosterStudentIds = buildEduCourseEffectiveRosterStudentIds({ plan, enrollments, students, migrationCompleted });
    const linkedSessionIds = new Set(linksSnap.docs.map((doc) => doc.id));
    const sessions = sessionsSnap.docs
      .map((doc) => ({ id: doc.id, ref: doc.ref, data: { id: doc.id, _docId: doc.id, ...(doc.data() || {}) } }))
      .sort((left, right) => {
        const leftMs = getEduCourseSessionStartMs(left.data) ?? Number.MAX_SAFE_INTEGER;
        const rightMs = getEduCourseSessionStartMs(right.data) ?? Number.MAX_SAFE_INTEGER;
        return (leftMs - rightMs) || left.id.localeCompare(right.id);
      });
    const updatedSessions = [];
    const linkedSyncSessions = [];
    const skipped = [];
    let eligibleCount = 0;
    let unchangedCount = 0;
    let pendingSessionCount = 0;
    const pushSkipped = (sessionId, reason) => {
      if (skipped.length < 20) skipped.push({ sessionId, reason });
    };
    for (const item of sessions) {
      const reason = getEduCourseSessionRosterSyncSkipReason(item.data, nowMs);
      if (reason) {
        pushSkipped(item.id, reason);
        continue;
      }
      eligibleCount += 1;
      if (eligibleCount > safeMaxSessions) {
        pendingSessionCount += 1;
        continue;
      }
      const currentIds = normalizeEduCourseRosterStudentIds(item.data.studentIds);
      const linkedEvent = linkedSessionIds.has(item.id);
      if (sameEduCourseRosterStudentIds(currentIds, rosterStudentIds)) {
        unchangedCount += 1;
        if (linkedEvent) linkedSyncSessions.push({
          sessionId: item.id,
          previousCount: currentIds.length,
          nextCount: rosterStudentIds.length,
          linkedEvent,
          unchanged: true,
        });
        continue;
      ...
      if (linkedEvent) linkedSyncSessions.push(updatedSession);
    }
    if (updatedSessions.length || linkedSyncSessions.length) {
      tx.update(planRef, {
        rosterSyncedAt: FieldValue.serverTimestamp(),
        rosterSyncRevision: FieldValue.increment(1),
        rosterSyncSource: safeSource,
      });
    }
    return {
      ...
      planId: safePlanId,
      planType,
      rosterCount: rosterStudentIds.length,
  ...
  if (syncResult?.skipped === true || syncResult?.success === false) return syncResult;

  const linkedSync = [];
  const linkedSyncErrors = [];
  for (const session of syncResult.linkedSyncSessions || []) {
    try {
      const result = await syncCourseLessonRosterToEventInternal({
        teamId: safeTeamId,
        planId: safePlanId,
        sessionId: session.sessionId,
        source: safeSource,
      });
      if (result?.success === false || result?.orphanCleanup?.success === false) {
        throw new Error(result.reason || result.orphanCleanup?.reason || "linked_event_roster_sync_failed");
      }
      linkedSync.push({
        sessionId: session.sessionId,
        eventId: result?.eventId || "",
        orphanCancelledCount: result?.orphanCancelledCount || 0,
        promotedCount: result?.promotedCount || 0,
      });
    } catch (err) {
      linkedSyncErrors.push({ sessionId: session.sessionId, message: err?.message || String(err) });
    }
  }
  return {
    ...syncResult,
    success: linkedSyncErrors.length === 0,
    reason: linkedSyncErrors.length ? "linked_event_roster_sync_failed" : null,
    linkedSyncCandidateCount: (syncResult.linkedSyncSessions || []).length,
    linkedSync,
    linkedSyncErrors,
  };
}
async function syncEduCoursePlanAutoSessionRostersForStudentChange({ teamId, beforeData, afterData, source = "course_student_write" } = {}) {
  const safeTeamId = sanitizeStr(teamId, 100);
  const groupIds = normalizeEduCourseRosterStudentIds([
    ...getEduCourseStudentGroupIds(beforeData || {}),
    ...getEduCourseStudentGroupIds(afterData || {}),
  ]);
  if (!safeTeamId || !groupIds.length) {
    return { success: true, skipped: true, reason: "no_course_groups", teamId: safeTeamId, groupIds };
  }
  const teamDoc = await getTeamDocByTeamId(safeTeamId);
  if (!teamDoc) return { success: false, reason: "team_not_found", teamId: safeTeamId, groupIds };
  const plansSnap = await teamDoc.ref.collection("coursePlans").get();
  const groupSet = new Set(groupIds);
  const targetPlanIds = plansSnap.docs
    .map((doc) => ({ id: doc.id, data: doc.data() || {} }))
    .filter((item) => groupSet.has(sanitizeStr(item.data.groupId, 100)))
    .filter((item) => ["weekly", "session"].includes(sanitizeStr(item.data.planType, 32)))
    .map((item) => item.id);
  const results = [];
  for (const targetPlanId of targetPlanIds) {
    results.push(await syncEduCoursePlanAutoSessionRostersInternal({
      teamId: safeTeamId,
      planId: targetPlanId,
      source,
    }));
  ...
  return {
    ...
    reason: failed.length ? "course_roster_sync_failed" : null,
    teamId: safeTeamId,
    groupIds,
    planCount: targetPlanIds.length,
    updatedCount: results.reduce((sum, result) => sum + (result?.updatedCount || 0), 0),
    results,
  };
}

async function resolveCourseCallerName(request, callerUid) {
  const found = await findUserDocByUidOrLineUserId(callerUid);
  let authDisplayName = "";
  try {
    const authUser = await authAdmin.getUser(callerUid);
    authDisplayName = getDisplayNameFromAuthRecord(authUser, "");
...
exports.syncEduCoursePlanAutoSessionRosters = onCall(
  { ...EDU_COURSE_ROSTER_SYNC_FUNCTION_OPTIONS, minInstances: 1 },
  async (request) => {
    if (!request.auth?.uid) {
      throw new HttpsError("unauthenticated", "Authentication required");
    }
    ...
    const result = await syncEduCoursePlanAutoSessionRostersInternal({
      teamId,
      planId,
      source: "syncEduCoursePlanAutoSessionRosters",
      maxSessions: request.data?.maxSessions,
    });
    if (result?.success === false) {
      throw new HttpsError("failed-precondition", "COURSE_SESSION_ROSTER_SYNC_FAILED", result);
    }
    return result;
  }
);
exports.onEduCourseEnrollmentRosterSourceWrite = onDocumentWritten(
  {
    ...EDU_COURSE_ROSTER_SYNC_TRIGGER_OPTIONS,
    document: "teams/{teamId}/coursePlans/{planId}/enrollments/{enrollmentId}",
  },
  async (event) => {
    const beforeData = event.data?.before?.exists ? (event.data.before.data() || {}) : null;
    const afterData = event.data?.after?.exists ? (event.data.after.data() || {}) : null;
    if (!hasEduCourseEnrollmentRosterRelevantChange(beforeData, afterData)) return null;
    const result = await syncEduCoursePlanAutoSessionRostersInternal({
      teamId: event.params.teamId,
      planId: event.params.planId,
      source: "course_enrollment_write",
    });
    if (result?.updatedCount || result?.success === false) {
      console.log("[courseEnrollmentRosterSync]", result);
    }
    if (result?.success === false) throw new Error(result.reason || "course_enrollment_roster_sync_failed");
    return result;
  }
);
exports.onEduCourseStudentRosterSourceWrite = onDocumentWritten(
  {
    ...EDU_COURSE_ROSTER_SYNC_TRIGGER_OPTIONS,
    document: "teams/{teamId}/students/{studentId}",
  },
  async (event) => {
    const beforeData = event.data?.before?.exists ? (event.data.before.data() || {}) : null;
    const afterData = event.data?.after?.exists ? (event.data.after.data() || {}) : null;
    if (!hasEduCourseStudentRosterRelevantChange(beforeData, afterData)) return null;
    const result = await syncEduCoursePlanAutoSessionRostersForStudentChange({
      teamId: event.params.teamId,
      beforeData,
      afterData,
      source: "course_student_write",
    });
    if (result?.updatedCount || result?.success === false) {
      console.log("[courseStudentRosterSync]", result);
    }
    if (result?.success === false) throw new Error(result.reason || "course_student_roster_sync_failed");
    return result;
  }
);
exports.onEduCoursePlanRosterSourceWrite = onDocumentWritten(
  {
    ...EDU_COURSE_ROSTER_SYNC_TRIGGER_OPTIONS,
    document: "teams/{teamId}/coursePlans/{planId}",
  },
  async (event) => {
    const beforeData = event.data?.before?.exists ? (event.data.before.data() || {}) : null;
    const afterData = event.data?.after?.exists ? (event.data.after.data() || {}) : null;
    if (!hasEduCoursePlanRosterRelevantChange(beforeData, afterData)) return null;
    const result = await syncEduCoursePlanAutoSessionRostersInternal({
      teamId: event.params.teamId,
      planId: event.params.planId,
      source: "course_plan_write",
    });
    if (result?.updatedCount || result?.success === false) {
      console.log("[coursePlanRosterSync]", result);
    }
    if (result?.success === false) throw new Error(result.reason || "course_plan_roster_sync_failed");
    return result;
  }
);
...
exports.registerForEduCoursePlan = onCall(
  ...
  async (request) => {
    ...
    const callerUid = request.auth.uid;
    const { teamId, planId } = normalizeEduCourseRequestIds(request.data || {});
    const requestedStudentIds = Array.isArray(request.data?.studentIds)
      ? request.data.studentIds
    ...
    return db.runTransaction(async (tx) => {
      ...
      if (!planSnap.exists) throw createEduCourseHttpsError("PLAN_NOT_FOUND");
      const plan = { id: planSnap.id, _docId: planSnap.id, ...(planSnap.data() || {}) };
      const [enrollments, students, migrationCompleted] = await Promise.all([
        loadCourseEnrollmentsForDecision(tx, planRef),
        loadCourseStudentsForDecision(tx, teamRef),
        loadEduAutoMigrationCompletedInTransaction(tx),
      ]);
      const studentsById = new Map(students.map((student) => [String(student.id || student._docId || ""), student]));
      const decision = decideCoursePlanRegistration({
        plan,
        enrollments,
        studentsById,
        requestedStudentIds,
        callerUid,
        isStaff,
        bypassCapacity: isStaff && request.data?.bypassCapacity === true,
        migrationCompleted,
        now: new Date(),
      });
      if (!decision.ok) throw createEduCourseHttpsError(decision.code, decision.code, decision);

      const nowTimestamp = Timestamp.now();
      const nowIso = nowTimestamp.toDate().toISOString();
      const created = [];
      decision.acceptedStudentIds.forEach((studentId) => {
        const student = studentsById.get(studentId);
        const enrollRef = planRef.collection("enrollments").doc(courseEnrollmentDocId(studentId));
        ...
        created.push({ ...payload, _docId: enrollRef.id, appliedAt: nowIso });
      });
      tx.update(planRef, {
...
exports.approveCourseEnrollment = onCall(
  ...
  async (request) => {
    ...
    const callerUid = request.auth.uid;
    const { teamId, planId } = normalizeEduCourseRequestIds(request.data || {});
    const enrollId = sanitizeStr(request.data?.enrollId || request.data?.enrollmentId, 100);
    if (!enrollId) throw new HttpsError("invalid-argument", "enrollId is required");
    const access = await getCallerAccessContext(request);
    const reviewerName = await resolveCourseCallerName(request, callerUid);
    ...
    return db.runTransaction(async (tx) => {
      ...
      const isStaff = isTeamStaffForData(teamDoc.data, callerUid)
        || access.isSuperAdmin
        || access.hasPermission("team.manage_all");
      if (!isStaff) throw createEduCourseHttpsError("PERMISSION_DENIED");
      ...
      const decision = decideCoursePlanApproval({
        plan,
        enrollments,
        students,
      ...
      if (decision.alreadyApproved) {
        return { success: true, alreadyApproved: true, teamId, planId, enrollId };
      }

      const enrollRef = planRef.collection("enrollments").doc(enrollId);
      ...
      const studentId = sanitizeStr(target.studentId, 100);
      if (studentId) {
        const studentRef = teamRef.collection("students").doc(studentId);
        const student = students.find((item) => String(item.id || item._docId || "") === studentId);
        if (student && student.enrollStatus === "pending") {
      ...
      return {
        ...
        planId,
        enrollId,
        approvedCountBefore: decision.approvedCount,
        maxCapacity: decision.maxCapacity,
      };
    });
  }
...
exports.migrateEduCourseAutoEnrollments = onCall(
  { region: "asia-east1", timeoutSeconds: 300, memory: "512MiB", minInstances: 0 },
  async (request) => {
    if (!request.auth?.uid) {
      throw new HttpsError("unauthenticated", "Authentication required");
    }
    const access = await getCallerAccessContext(request);
    if (!hasEduCourseMigrationAccess(access)) {
    ...
    const limitPlans = normalizeEduCourseMigrationLimit(request.data?.limitPlans, planId ? 1 : 100, 1, 300);
    const limitCandidates = normalizeEduCourseMigrationLimit(request.data?.limitCandidates, dryRun ? 1000 : 400, 1, dryRun ? 5000 : 1000);
    const sampleLimit = normalizeEduCourseMigrationLimit(request.data?.sampleLimit, 20, 0, 100);

    if (markCompleted && dryRun) {
      throw new HttpsError("failed-precondition", "markCompleted requires dryRun:false");
    }
    if (markCompleted && (teamId || planId)) {
      throw new HttpsError("failed-precondition", "markCompleted must run without teamId/planId scope");
    }
    ...
    const ensureBatchCapacity = async (ops = 1) => {
      if (!dryRun && batchOps + ops >= 450) await flushBatch();
    };

    outer:
    for (const teamDoc of teamDocs) {
      stats.scannedTeams += 1;
      const safeTeamId = sanitizeStr(teamDoc.data?.id || teamDoc.docId, 100);
      ...
      for (const planDoc of planDocs) {
        ...
        const plan = { id: planDoc.id, _docId: planDoc.id, ...(planDoc.data() || {}) };
        const groupId = sanitizeStr(plan.groupId, 100);
        if (!groupId) {
          stats.skipped.noGroup += 1;
          continue;
        }
        stats.plansWithGroup += 1;

        const enrollSnap = await planDoc.ref.collection("enrollments").get();
        const existingStudentIds = new Set();
        enrollSnap.docs.forEach((doc) => {
          const data = doc.data() || {};
          const studentId = sanitizeStr(data.studentId, 100);
        ...
        for (const student of students) {
          ...
          const studentId = sanitizeStr(student.id || student._docId, 100);
          if (!studentId) {
            stats.skipped.invalidStudentId += 1;
            continue;
          }
          const groupIds = Array.isArray(student.groupIds)
            ? student.groupIds.map((value) => sanitizeStr(value, 100))
            : [];
          if (student.enrollStatus !== "active" || !groupIds.includes(groupId)) {
            stats.skipped.inactiveStudent += 1;
            continue;
          }
          if (existingStudentIds.has(studentId)) {
            stats.skipped.existingEnrollment += 1;
            continue;
          }
          ...
          const sample = {
            teamId: safeTeamId,
            teamDocId: teamDoc.docId,
            planId: planDoc.id,
            groupId,
            studentId,
            studentName: sanitizeStr(student.name, 80),
          };
          ...
          const payload = {
            ...
            parentUid: sanitizeStr(student.parentUid, 128) || null,
            status: "approved",
            enrollmentType: "auto_group_migrated",
            migrationSource: "group_auto",
          ...
          await ensureBatchCapacity(1);
          batch.create(enrollRef, payload);
          batchOps += 1;
          stats.written += 1;
    ...
    if (markCompleted) {
      if (stats.truncated) {
        throw new HttpsError("failed-precondition", "Cannot mark completed while migration scan is truncated", stats);
      }
      await db.collection("siteConfig").doc("featureFlags").set({
        eduAutoMigrationCompleted: true,
        eduAutoMigrationCompletedAt: FieldValue.serverTimestamp(),
        eduAutoMigrationCompletedByUid: callerUid,
      }, { merge: true });
