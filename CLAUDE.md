...
> **Last Reviewed / Verified: 2026-07-17**
...
- **R-MODE-004 [A/HARD]**：本節矩陣、R-SHIP-010、R-SHIP-016～018 屬 A/HARD；R-SHIP-011～015、R-SHIP-019 依各自標示為 B/HARD。同步文件／日誌、清理、保守解讀或 release preparation 都不得越過當前模式；READ_ONLY、DIAGNOSE、PLAN、REVIEW 未獲修改授權時永不寫檔。
...
| cache 版號不同步 | 新舊 JS／HTML 混載 | R-REL-001～R-REL-009 |
...
| 獨立內容型 HTML（只改文案／連結／metadata／JSON-LD 事實） | R-REL-009、R-QA-010～013、R-ENC-001～007 | 不 bump；跑相關 static／SEO tests，必要時 browser |
...
- **R-REL-009 [B/HARD]**：`manual.html`、`privacy.html`、`terms.html`、`permissions/`、`roles/`、`blog/`、`seo/` 等獨立 network-first 內容頁，若只改可見文案、連結、metadata 或 JSON-LD 事實，且不改 executable inline JS／CSS、shared asset、route、互動或 app shell，視為內容文件：不更新主站 cache 版號；仍須跑相關 static／SEO tests，並依 R-QA-010～013 檢查受影響頁面。超出此邊界即回到 R-REL-006。
...
- **R-SHIP-003 [B/HARD]**：純 Markdown／local-only docs，以及符合 R-REL-009 的內容型 HTML，可免主站 bump 與完整 code review；HTML 仍需相稱 static／SEO／browser 驗證。所有文件仍需 diff／encoding 與 commit／push 授權。
...
- **R-IAM-003 [B/HARD]**：改 `DRAWER_MENUS`、`ADMIN_PAGE_EXTRA_PERMISSION_ITEMS`、`ROLE_ACTIVITY_CAPABILITY_ITEMS`、`rolePermissions`、`roleActivityCapabilities` 時，更新權限測試並查 `js/modules/user-admin/permission-audit/` 覆蓋所有角色、入口、子權限、user capabilities、高風險組合。
- **R-IAM-004 [A/HARD]**：`event.edit_all` 是跨全活動編輯唯一總開關；無它只能在 owner／delegate record scope 內依 action-specific helper、permission 或 capability 執行被明確允許的操作。`activity.manage.entry` 的目標語意只代表入口可見，禁止新路徑把它當泛用動作授權。**已知 drift**：前端 `_hasActivityManageEntry()` 與 Rules `hasActivityManageEntry()` 仍含 coach+ 角色 fallback，且 Rules 會把它用於 scoped event update；Functions `getCallerAccessContext()` 會合併個別授權，而 `canOperateEventSiteForAccess()`／`canManageSingleEventRosterForAccess()` 對 coach+、`activity.manage.entry` 或 `event.edit_all` 直接回傳跨活動通過，未驗 owner／delegate scope，`cancelRegistration` 的 `manager_remove`／`capacity_change` 會使用此路徑。因此關閉 staff 的 `rolePermissions` 目前不能視為已完整撤回後端 scoped activity 能力；一般 user 個別取得 `activity.manage.entry` 也可能在 Callable 取得超出入口語意的跨活動名單操作。這些都是待修授權 drift，不得複製或當目標契約。
...
- **R-IAM-008 [A/HARD]**：`INHERENT_ROLE_PERMISSIONS` 同時在 `js/config.js`、`functions/index.js`；改任一邊同步另一邊並驗 parity。coach／captain／venue_owner 的陣列為空，不代表後端已可完整撤權；權限稽核必須同時檢查 `firestore.rules#hasActivityManageEntry()` 的 `isCoachPlus()` fallback，以及 Functions `getCallerAccessContext()` → `hasActivityManageEntryAccess()` → `canOperateEventSiteForAccess()`／`canManageSingleEventRosterForAccess()` 的 scope。修正此 drift 時同步前端、Functions、Rules 與 tests，覆蓋 coach+、角色權限關閉、一般 user 個別 grant、非 owner／delegate 的 allow／deny。
- **R-IAM-009 [B/HARD]**：查看類 render 預設登入用戶可見；編輯、刪除、簽到才守衛。
- **R-IAM-010 [A/HARD]**：禁只寫 `if (!hasPermission(...)) return`，也禁把 `.entry` 或泛用 `_canManageEvent(e)` 當動作授權。跨活動編輯只認 `event.edit_all`；編輯依 `_canEditOwnActivityBasic`，取消依 `_canCancelOwnActivity`，結束／重開／重新上架依 `_canReopenOrRelistActivity`，刪除依 `_canDeleteActivity`，現場操作依 `_canOperateEventSite`，正取移除再依 `_canRemoveConfirmedParticipant`。delegate 只界定 record scope，不是獨立 action grant；每個 helper 仍須依角色 permission／user capability／明列 fallback 判斷。拒絕時 Toast 後 return。
- **R-IAM-011 [A/HARD]**：按鈕顯示與 handler 使用同一 action-specific permission／helper；禁止一寬一窄。
- **R-IAM-012 [B/HARD]**：逐動作測 `user`、`coach`、`captain`、owner、delegate；一般 user delegate 同時覆蓋有／無 `own_edit_basic`、`own_cancel`、`site_operate`，並分開驗編輯、取消、結束、重開、現場操作、正取移除與刪除。
- **R-IAM-013 [A/HARD]**：現行 delegate 契約：一般 user delegate 有 `user.activity.own_edit_basic`／`own_cancel` 時可編輯基本資料／取消；明列 delegate 即使沒有 `site_operate` 仍可進 `_canOperateEventSite` 的現場操作，但 `_canRemoveConfirmedParticipant` 仍須額外條件；staff delegate 因目前 entry／coach+ fallback 可編輯、取消、結束與重開／重新上架。delegate 身分本身不授予刪除，也不授予設定其他 delegate；刪除只依 `_canDeleteActivity`，delegate 管理只限 owner 或全域管理者。
...
- **R-CAP-002 [A/HARD]**：user capability 只控制自己主辦／受委託的 scoped activity，不改全域權限；Rules 用 `hasActivityCap(...)`。明列 delegate 的現場操作 fallback 是獨立 legacy contract：前端 `_canOperateEventSite` 與 Rules `isEventSiteOperatorForData` 可在沒有 `site_operate` 時接受 delegate，但不因此取得正取移除、編輯、取消或刪除；各動作仍依 R-IAM-010～013。
...
| `user.activity.own_edit_basic` | 編自己主辦或受委託活動的基本資料 |
| `user.activity.own_cancel` | 取消自己主辦或受委託活動 |
| `user.activity.site_operate` | owner-scope 現場簽到／候補；delegate 例外見 R-IAM-013 |
| `user.activity.delegate_assign` | owner 設自己活動委託人 |
...
- **R-EVT-018 [A/HARD]**：code `admin.repair.event_blocklist`；`super_admin` inherent。`rolePermissions/user` 固定不授予；角色層 toggle 仍顯示但鎖定／disabled，不能透過 `rolePermissions` 開啟。`super_admin` 可透過 `userPermissionGrants/{uid}` 個別授予特定一般 user；其他非 user 角色可透過 `rolePermissions` 調整，也可接受個別授權。所有入口一律以目前登入者的 effective permission 判斷。
...
- **R-REG-014 [A/HARD]**：`registrations`、`attendanceRecords`、`activityRecords` 的 canonical 路徑一律是 `events/{eventDocId}/{collection}`，禁止新增 root collection 查詢／寫入。單活動先 resolve `eventDocId` 再讀對應子集合；跨活動用 `collectionGroup(collectionName)`，前端保留 `doc.ref.parent.parent !== null`，Admin／CF 保留 `d.ref.path.split('/').length > 2` 或等價共用 predicate。Phase 4c legacy root 資料尚未完整移除前，三種 collection 的排除 predicate 都不得刪。 已知待修 drift：`js/modules/home-next-activity.js#fetchRegistrationsForUid()` 目前把 `collectionGroup('registrations')` 結果直接映射，未套 ancestry predicate，且以 `requireSubcollection:false` 寫入 cache，可能混入 legacy root 文件；不得把此路徑當正確先例。後續 code task 必須補 predicate／cache guard 與 legacy-root allow／deny 測試；本次 docs-only release 不改 runtime。
...
- **R-STAT-003 [A/HARD]**：`attendanceRecords.uid`、`activityRecords.uid` 的 canonical 身分是 LINE／Firebase UID；新 join 只直接比 UID，禁止新增 displayName fallback。**既有 legacy 例外**：`js/modules/achievement/batch.js#_processOneUserAchievements` 在 UID attendance 查無資料時仍以 `userName == displayName` 補查並映射 UID；此路徑有同名碰撞風險，不得複製，只有完成歷史資料驗證／遷移與相稱 tests 後才能移除。
...
- **R-NAV-011 [A/HARD]**：一般 `history.replaceState/pushState` 完整 state 為 `{ source:'sportshub', pageId:'page-xxx', id?, fallbackPageId? }`；sentinel 的 canonical 例外為 `{ source:'sportshub', sentinel:true, fallbackPageId:'page-home' }`，可不含 `pageId`。目標契約禁止 null state。**已知 drift**：`app.js#_clearDeepLinkQueryParams` 與 `index.html` inline 同名實作在 `preserveHistoryState:true` 且 `history.state` 為空時仍會 `replaceState(null, ...)`；不得複製，專門修復時改用有效 current-route state fallback 並跑 history／route tests。
...
| 每次修改即 bump vs release batch／獨立內容頁 | R-REL-006～009 | POLICY_CLARIFIED |
...
| 委託人僅簽到／掃碼 vs action-specific edit／cancel／close／reopen scope | R-IAM-004、R-IAM-010～013、R-CAP-002 | STALE_CONTRACT_CORRECTED |
| staff rolePermissions 可完整撤權 vs Rules `isCoachPlus()`／Functions 跨活動 fallback | R-IAM-004、R-IAM-008 | RUNTIME_DRIFT_DISCLOSED |
| 三種活動紀錄子集合契約被縮成 registrations | R-REG-014 | SEMANTIC_RESTORED |
| collectionGroup 必須排除 legacy root vs home next registrations 漏 filter | R-REG-014 | RUNTIME_DRIFT_DISCLOSED |
| 統計只准 UID vs achievement batch 歷史姓名補查 | R-STAT-003 | LEGACY_EXCEPTION_DISCLOSED |
| history 全路徑 non-null vs preserve-state null fallback | R-NAV-011 | RUNTIME_DRIFT_DISCLOSED |
| R-SHIP authority 區間與逐條標記不一致 | R-MODE-004、R-SHIP-010～019 | CONFLICT_RESOLVED |
| 一般 user 活動黑名單「絕對無」vs UID 個別授權 | R-EVT-018～019 | AUTH_SCOPE_CORRECTED |
